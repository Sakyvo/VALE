...
The resumable index is `data/internal/pack-content-index.json`. The scan downloads each archive to an OS temp directory and deletes it after hashing. Existing exact groups are reported in `docs/PACK_CONTENT_DUPLICATES.md`; the command never deletes them.
